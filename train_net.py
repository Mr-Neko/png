...
def SAL(image_layer, lang, gt_mask:torch.Tensor, grad_sample):
    ...
    image_layer = image_layer.view(bsz, -1, h*w).permute(0, 2, 1)

    gt_mask = F.interpolate(gt_mask, scale_factor=0.5, mode='bilinear')
    gt_mask = (gt_mask > 0).float()
    loss = 0
    for i in range(bsz):
        gt = gt_mask[i, grad_sample[i]]

        text_len = gt.shape[0]
        gt = gt.view(text_len, -1) # l, h*w

        image = image_layer[i].permute(1, 0)  # c, h*w

        text = lang[i, grad_sample[i]] # l, c

        matrix = torch.matmul(text, image) # l, h*w
        ...
        loss = loss + loss_1 * 0.5 + loss_2 * 0.5
    return loss / bsz
...
def train_epoch(train_loader, bert_encoder, fpn_model, model, 
    ...
    for (batch_idx, (caption, grounding_instances, ann_categories, \
        ...
        image_layer, out, predictions = model(fpn_feature, lang_feat_valid, ann_types) #Knet
