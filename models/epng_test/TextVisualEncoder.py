    ...
    def forward(self, inputs, encoder_output, gt_mask, mask):
        ...
        for i, l in enumerate(self.layers):
            ...
            if mask is None:
                ...
                attn_map = (attn_map.view(b, n, -1).unsqueeze(dim=1).expand(b, self.h, n, h*w)) < (0.2 + i * 0.1)
            '''
            

            out = l(out, encoder_output.view(b, -1, h*w).permute(0, 2, 1), [h, w], None, None)
            _, mask = self._map(encoder_output, out, i)
